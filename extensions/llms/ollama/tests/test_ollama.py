...
def create_response(content, mode="chat"):
    if mode == "chat":
        ...
        return SimpleNamespace(
            choices=[SimpleNamespace(text=content)]
        )
...
class TestOllamaLLM:
    def test_default_base_url(self, prompt):
        ...
        with mock.patch("extensions.llms.ollama.pandasai_ollama.ollama.openai.OpenAI") as mock_openai:
            dummy_client = mock.Mock()
            dummy_completion = mock.Mock()
            expected_response = "Ollama dummy response"
            dummy_completion.create.return_value = create_response(expected_response, mode="chat")
    ...
    def test_params_setting(self, prompt):
        with mock.patch("extensions.llms.ollama.pandasai_ollama.ollama.openai.OpenAI") as mock_openai:
            dummy_client = mock.Mock()
            dummy_completion = mock.Mock()
            dummy_completion.create.return_value = create_response("Dummy response", mode="chat")
            dummy_client.chat.completions = dummy_completion
            mock_openai.return_value = dummy_client
    
            llm = OllamaLLM(api_key="dummy", model="llama2", temperature=0.7, max_tokens=150)
    ...
    def test_call_non_chat_mode(self, prompt):
        ...
        with mock.patch("extensions.llms.ollama.pandasai_ollama.ollama.openai.OpenAI") as mock_openai:
            ...
            dummy_completion.create.return_value = create_response(expected_response, mode="non-chat")
